...
static t_ast_node	*ast_command(t_token **tokens)
{
	t_token		*redir_tokens[256];  // Arbitrary limit
	int			redir_count = 0;
	char		**data;
	t_ast_node	*cmd;
	int			i;

	// Step 1: Accumulate redirections (even before command)
	while (*tokens && (
		(*tokens)->type == TOKEN_REDIRECTION_IN ||
		...
		(*tokens)->type == TOKEN_HEREDOC))
	{
		...
		redir_tokens[redir_count++] = *tokens;     // save operator
		redir_tokens[redir_count++] = (*tokens)->next; // save file
		*tokens = (*tokens)->next->next;
	}

	// Step 2: Parse actual command
	i = 0;
	t_token *tmp = *tokens;
	...
	// Step 3: Attach redirections in reverse order (last in, first out)
	for (int j = redir_count - 2; j >= 0; j -= 2)
	{
		t_node_types type = redirection_type(redir_tokens[j]);
		char **redir_data = malloc(sizeof(char *) * 2);
		if (!redir_data)
			return (NULL);
